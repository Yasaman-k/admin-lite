<template>
  <section class="container my-16 px-20">
    <FormHeader
      linkText="Upload your <a href='https://www.myscoreiq.com/' target='_blank' class='underline'> MyScoreIQ.com </a> Personal Credit report"
      desc="To ensure accurate reporting and your best match for business credit and lending sources, our system only accepts Personal Credit reports from <a href='https://www.myscoreiq.com/' target='_blank' class='underline'> MyScoreIQ.com. </a>"
    />
    <p class="mt-10 text-base font-bold">
      Upload
      <a href="https://www.myscoreiq.com/" target="_blank" class="underline">
        MyScoreIQ.com
      </a>
      report
    </p>
    <BaseDropZone
      class="bg-[#fafdff] w-3/4 h-[265px] mt-10 rounded-sm custom-border custom-drop-shadow"
...
// Hooks
onBeforeMount(async () => {
  await store.dispatch("creditReport/fetchCreditReportFile");
  const fileData = store.getters["creditReport/getFileData"];
...
const uploadFile = async () => {
  if (files.value[0]) {
    ...
    await store.dispatch("creditReport/sendCreditReportFile", formData);
    const fileData = store.getters["creditReport/getFileData"];
    files.value = [];
