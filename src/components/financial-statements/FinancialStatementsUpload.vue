<template>
  <section class="container my-16 px-20">
    <FormHeader
      linkText="Upload your Financial Statements"
      desc="Please upload your most recent Profit and Loss Statement and Balance Sheet to ensure your best match for business credit and lending recommendations."
    />
    <p class="mt-10 text-base font-semibold">Upload Financial Report</p>
    <BaseDropZone
...
// Hooks
onBeforeMount(async () => {
  await store.dispatch("financialStatement/fetchFinancialStatementFile");
...
const uploadFile = async () => {
  if (files.value[0]) {
    ...
    await store.dispatch(
      "financialStatement/sendFinancialStatementFile",
      formData
    );
    const fileData = store.getters["financialStatement/getFileData"];
    files.value = [];
