<template>
  <section class="container my-16 px-20">
    <FormHeader
      linkText="Upload your LexisNexis report"
      desc="To ensure your best match for business credit and lending recommendations, upload your LexisNexis report below."
    />
    <p class="mt-10 text-base font-semibold">Upload LexisNexis report</p>
    <BaseDropZone
...
// Hooks
onBeforeMount(async () => {
  await store.dispatch("lexisNexis/fetchLexisNexisFile");
  const fileData = store.getters["lexisNexis/getFileData"];
...
const uploadFile = async () => {
  if (files.value[0]) {
    ...
    await store.dispatch("lexisNexis/sendLexisNexisFile", formData);
    const fileData = store.getters["lexisNexis/getFileData"];
    files.value = [];
...
const next = async () => {
  ...
  await store.dispatch("aspects/fetchPersonalStatuses", ["lexisnexis_report"]);
